...
class Instance extends common.GrpcServiceObject {
        ...
        this.getMetadata(getMetadataOptions, (err, metadata) => {
            if (err) {
                if (err.code === 5 && options.autoCreate) {
                    this.create(options, (err, instance, operation) => {
                        if (err) {
                            callback(err);
                            return;
                        }
     ...
     * });
     */
    getDatabases(queryOrCallback, cb) {
        const self = this;
        const callback = typeof queryOrCallback === 'function' ? queryOrCallback : cb;
        const query = typeof queryOrCallback === 'object'
            ? queryOrCallback
            : {};
 ...
 *   });
 */
Instance.prototype.getDatabasesStream = paginator_1.paginator.streamify('getDatabases');
